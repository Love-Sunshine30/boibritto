...
func (s *Store) GetUserByFirebaseUID(ctx context.Context, firebaseUID string) (*domain.User, error) {
	...
	`, firebaseUID).Scan(&u.ID, &u.FirebaseUID, &u.Name, &u.Email, &u.WhatsAppNumber,
		&u.LastActiveAt, &u.IsAdmin, &u.CreatedAt)
...
func (s *Store) InsertUser(ctx context.Context, firebaseUID, name, email string) (*domain.User, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		...
		VALUES ($1, $2, $3)
		RETURNING id, firebase_uid, name, email, whatsapp_number, last_active_at, is_admin, created_at
	`, firebaseUID, name, email).Scan(&u.ID, &u.FirebaseUID, &u.Name, &u.Email, &u.WhatsAppNumber, &u.LastActiveAt, &u.IsAdmin, &u.CreatedAt)
